...
int     whichroom;
{
	...
	while (*msg && !isdigit(*msg))
		msg++;
	stop = atoi(msg);
	send_to_unum(unum, "[1;33;44m ÁÄÌì´úºÅ  Ê¹ÓÃÕß´úºÅ    ÁÄ    Ìì    ÊÒ  Op  À´×Ô                          [m");
	for (i = 0; i < MAXROOM; i++) {
		rnum = users[i].room;
...
char   *msg;
{
	...
	if (recunum < 0) {
		...
		if (recunum == -1)
			...
			sprintf(chatbuf, "[1;31m ¡ò[37m ÄÇÎ»ÈşÓëÕß½ĞÊ²÷áÃû×Ö [31m¡ò[m");
		send_to_unum(unum, chatbuf);
		return;
	}
	if (*msg) {
...
{
	if (!SYSOP(unum)) {
		send_to_unum(unum, "[1;31m¡ò [37mÄã²»¿ÉÒÔÔÚ»áÒéÌüÄÚ´óÉùĞúßÙ [31m¡ò[m");
		return;
	}
	if (*msg == '\0') {
...
struct action condition_data[] =
{
	...
	{"luck", "ÍÛ£¡¸£ÆøÀ²£¡", NULL},
	{"puke", "Õæ  ĞÄ£¬ÎÒÌıÁË¶¼ÏëÍÂ", NULL},
	{"shake", "Ò¡ÁËÒ¡Í·", NULL},
	{"sleep", "Zzzzzzzzzz£¬ÕæÎŞÁÄ£¬¶¼¿ìË¯×ÅÁË", NULL},
	{"so", "¾Í½´×Ó!!", NULL},
	{"strut", "´óÒ¡´ó°ÚµØ×ß", NULL},
	{"tongue", "ÍÂÁËÍÂÉàÍ·", NULL},
	{"think", "Íá×ÅÍ·ÏëÁËÒ»ÏÂ", NULL},
	{NULL, NULL, NULL}
};
