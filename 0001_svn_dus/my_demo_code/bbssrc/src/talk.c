...
*/
/*
$Id: talk.c,v 1.21 1999/08/30 14:00:21 edwardc Exp $
*/
...
char    q_id[IDLEN + 2];
{
	...
	prints("[1;37m%s [m([1;33m%s[m) ¹²ÉÏÕ¾ [1;32m%d[m ´Î£¬·¢±í¹ý [1;32m%d[m ÆªÎÄÕÂ",
	...
	prints("ÐÅÏä£º[[1;5;32m%2s[m]£¬¾­ÑéÖµ£º[[1;32m%d[m]([1;33m%s[m) ±íÏÖÖµ£º[[1;32m%d[m]([1;33m%s[m) ÉúÃüÁ¦£º[[1;32m%d[m]¡£\n"
		,(check_query_mail(qry_mail_dir) == 1) ? "¡Ñ" : "  ", exp, cexp(exp), perf,
...
{
/* Sonny.990514 ÊÔ×Å×¥ overflow µÄÎÊÌâ... */
/* Sonny.990606 overflow ÎÊÌâ½â¾ö. buf[100] ÊÇÕýÈ·µÄ. Èþ¿¼ man sprintf() */
	time_t  now;
	char    buf[100];
	now = time(0);
	if (msg[strlen(msg)] == '\n')
